...
class DbImages(QObject):
    ...
    def run(self) -> dict:
        ...
        q = sqlalchemy.select(
            _table_thumbs.c.short_hash, # relative thumb path
        ...
        return {
            rel_thumb_path: (
                Utils.get_abs_any_path(mf_path, rel_path),
                size,
                birth,
                mod
            )
...
class DbUpdater(QObject):
    ...
    def run_del_items(self):
        ...
        for rel_thumb_path in self.del_items:
            q = sqlalchemy.delete(_table_thumbs)
            q = q.where(_table_thumbs.c.short_hash==rel_thumb_path)
            q = q.where(_table_thumbs.c.brand==self.mf.alias)
            try:
                conn.execute(q)
    ...
    def run_new_items(self):
        ...
        for path, size, birth, mod in self.new_items:
            ...
            short_path = Utils.get_rel_any_path(self.mf.curr_path, path)
...
class MfRemover(QObject):
    ...
    def run(self):
        q = sqlalchemy.select(_table_thumbs.c.brand).distinct()
    ...
    def get_rows(self, mf_name):
        q = sqlalchemy.select(_table_thumbs.c.id, _table_thumbs.c.short_hash) #rel thumb path
        q = q.where(_table_thumbs.c.brand == mf_name)
    ...
    def remove_rows(self, rows: list):
        ...
        for id_, thumb_path in rows:
            q = sqlalchemy.delete(_table_thumbs)
            q = q.where(_table_thumbs.c.id == id_)
